...
expression <- c("is.na(INN$PUR_MES_SAME) == FALSE & INN$PUR_MES_SAME == 1", 
                "is.na(INN$PUR_MES_NEW) == FALSE & INN$PUR_MES_NEW == 1")

expression2 <- c("PUR_MES_SAME", 
                 "PUR_MES_NEW")
...
expression <- c("is.na(NINN$PUR_MES_SAME) == FALSE & NINN$PUR_MES_SAME == 1", 
                "is.na(NINN$PUR_MES_NEW) == FALSE & NINN$PUR_MES_NEW == 1")
...
result_T8_DT <- rbind(Aggregate_INN, Aggregate_NINN)
result_T8_DT %>% group_by(DATAFLOW, FREQ, TIME_PERIOD, REF_AREA, TABLENAME, ACTIVITY, NUMBER_EMPL, INN_PF, INDICATOR, CIS_INDICATOR, UNIT_MEASURE, UNIT_MULT, DECIMALS) %>% summarise(OBS_VALUE = sum(OBS_VALUE)) -> result_T8_DT
# result_T8_DT <- result_T8_DT[, .(SUM_OBS_VALUE=sum(OBS_VALUE)), by = "DATAFLOW,FREQ,TIME_PERIOD,REF_AREA,TABLENAME,ACTIVITY,NUMBER_EMPL,INN_PF,INDICATOR,CIS_INDICATOR,UNIT_MEASURE,UNIT_MULT,DECIMALS"]
colnames(result_T8_DT)[14] <- "OBS_VALUE"

result_T8_DT_FINAL <- cbind(result_T8_DT[, 1:7], stringsAsFactors = FALSE)
result_T8_DT_FINAL$TYPE_ENT <- "_T"
result_T8_DT_FINAL <- cbind(result_T8_DT_FINAL, result_T8_DT[, c(8:10, 14, 11:13)], stringsAsFactors = FALSE)
result_T8_DT_FINAL$OBS_STATUS <- ""
result_T8_DT_FINAL$OBS_STATUS_1 <- ""
result_T8_DT_FINAL$CONF_STATUS <- ""
result_T8_DT_FINAL$COMMENT_OBS <- ""
result_T8_DT_FINAL <- as.data.frame(result_T8_DT_FINAL)
result_T8_DT_FINAL$OBS_VALUE <- as.character(result_T8_DT_FINAL$OBS_VALUE)
result_T8_DT_FINAL[, "OBS_VALUE"] <- gsub("\\.", ",", result_T8_DT_FINAL[, "OBS_VALUE"])

result_T8_DT_FINAL[result_T8_DT_FINAL$ACTIVITY == "A" | result_T8_DT_FINAL$ACTIVITY == "A01" | result_T8_DT_FINAL$ACTIVITY == "A02" | result_T8_DT_FINAL$ACTIVITY == "A03" | result_T8_DT_FINAL$ACTIVITY == "F" | result_T8_DT_FINAL$ACTIVITY == "G45" | result_T8_DT_FINAL$ACTIVITY == "G47" | result_T8_DT_FINAL$ACTIVITY == "I" | result_T8_DT_FINAL$ACTIVITY == "L" | result_T8_DT_FINAL$ACTIVITY == "M69" | result_T8_DT_FINAL$ACTIVITY == "M70" | result_T8_DT_FINAL$ACTIVITY == "M74" | result_T8_DT_FINAL$ACTIVITY == "M75" | result_T8_DT_FINAL$ACTIVITY == "N" | result_T8_DT_FINAL$ACTIVITY == "N77" | result_T8_DT_FINAL$ACTIVITY == "N78" | result_T8_DT_FINAL$ACTIVITY == "N79" | result_T8_DT_FINAL$ACTIVITY == "N80" | result_T8_DT_FINAL$ACTIVITY == "N81" | result_T8_DT_FINAL$ACTIVITY == "N82", "OBS_VALUE"] <- ""
result_T8_DT_FINAL <- subset(result_T8_DT_FINAL, ACTIVITY != "GTN" | (ACTIVITY == "GTN" & (NUMBER_EMPL == "_T")))
# result_T8_DT_FINAL <- subset(result_T8_DT_FINAL, ACTIVITY != "G" & ACTIVITY != "M")
...
result_T8_DT_FINAL[result_T8_DT_FINAL$ACTIVITY == "G" | result_T8_DT_FINAL$ACTIVITY == "M", "OBS_VALUE"] <- ""
...
write.table(rbind(result_T8_DT_FINAL, Aggregate_INN, Aggregate_NINN), T8_SDMX_FINAL_RESULT, sep = ";", quote = FALSE, row.names = FALSE, append = FALSE)

set_ENT_Profile("as.numeric(PUR_MES_NEW)", "T08", T8_SDMX_FINAL_RESULT)
set_ENT_Profile("as.numeric(PUR_MES_SAME)", "T08", T8_SDMX_FINAL_RESULT)