...
result_T24_DT <- rbind(Aggregate_INN, Aggregate_NINN)
result_T24_DT %>% group_by(DATAFLOW, FREQ, TIME_PERIOD, REF_AREA, TABLENAME, ACTIVITY, NUMBER_EMPL, INN_PF, INDICATOR, CIS_INDICATOR, UNIT_MEASURE, UNIT_MULT, DECIMALS) %>% summarise(OBS_VALUE = sum(OBS_VALUE)) -> result_T24_DT
...
colnames(result_T24_DT)[14] <- "OBS_VALUE"
...
result_T24_DT_FINAL <- subset(result_T24_DT_FINAL, TYPE_ENT == "_T")
...
result_T24_DT_FINAL[result_T24_DT_FINAL$ACTIVITY == "G" | result_T24_DT_FINAL$ACTIVITY == "M", "OBS_VALUE"] <- ""
...
write.table(rbind(result_T24_DT_FINAL, Aggregate_INN, Aggregate_NINN), T24_SDMX_FINAL_RESULT, sep = ";", quote = FALSE, row.names = FALSE, append = FALSE)
...
write.table(rbind(result_T24_DT_FINAL, Aggregate_INN, Aggregate_NINN), T24_SDMX_FINAL_RESULT, sep = ";", quote = FALSE, row.names = FALSE, col.names = FALSE, append = TRUE)
