...
result_T22_DT <- rbind(Aggregate_INN, Aggregate_NINN)
result_T22_DT %>% group_by(DATAFLOW, FREQ, TIME_PERIOD, REF_AREA, TABLENAME, ACTIVITY, NUMBER_EMPL, INN_PF, INDICATOR, CIS_INDICATOR, UNIT_MEASURE, UNIT_MULT, DECIMALS) %>% summarise(OBS_VALUE = sum(OBS_VALUE)) -> result_T22_DT
...
colnames(result_T22_DT)[14] <- "OBS_VALUE"
...
result_T22_DT_FINAL <- subset(result_T22_DT_FINAL, TYPE_ENT == "_T")
...
result_T22_DT_FINAL[result_T22_DT_FINAL$ACTIVITY == "G" | result_T22_DT_FINAL$ACTIVITY == "M", "OBS_VALUE"] <- ""
...
write.table(rbind(result_T22_DT_FINAL, Aggregate_INN, Aggregate_NINN), T22_SDMX_FINAL_RESULT, sep = ";", quote = FALSE, row.names = FALSE, append = FALSE)
...
for(num in 1:length(expression)){
  ...
  if(nrow(INN_DT) != 0){
    ...
    INN_DT %>% mutate(TOVT_SULYOZOTT = eval_tidy(parse_expr(expression3[num])) / 100 * TOVT * VGMA001_SULY) -> INN_DT
...
for(num in 1:length(expression)){
  ...
  if(nrow(NINN_DT) != 0){
    ...
    NINN_DT %>% mutate(TOVT_SULYOZOTT = eval_tidy(parse_expr(expression3[num])) / 100 * TOVT * VGMA001_SULY) -> NINN_DT
...
write.table(rbind(result_T22_DT_FINAL, Aggregate_INN, Aggregate_NINN), T22_SDMX_FINAL_RESULT, sep = ";", quote = FALSE, row.names = FALSE, col.names = FALSE, append = TRUE)
