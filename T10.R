...
expression <- c("INN$M092 %in% TUR_PRD_NEW$M092",
                "INN$M092 %in% TUR_PRD_NEW_MKT$M092", 
                "INN$M092 %in% TUR_PRD_NEW_ENT$M092")

expression2 <- c("TUR_PRD_NEW",
                 "TUR_PRD_NEW_MKT", 
                 "TUR_PRD_NEW_ENT")
...
for(num in 1:length(expression)){
  ...
  if(nrow(INN_DT) != 0){
    ...
    INN_DT %>% mutate(TOVT_SULYOZOTT = eval_tidy(parse_expr(expression2[num])) / 100 * TOVT * VGMA001_SULY) -> INN_DT
    INN_DT %>% group_by(M065_RETEG1, M0581_2J) %>% summarise(TOVT_SULYOZOTT = sum(TOVT_SULYOZOTT)) %>% arrange(M065_RETEG1, M0581_2J) -> INN_Ordered
...
expression <- c("NINN$M092 %in% TUR_PRD_NEW_MKT$M092", 
                "NINN$M092 %in% TUR_PRD_NEW_ENT$M092")
...
for(num in 1:length(expression)){
  ...
  if(nrow(NINN_DT) != 0){
    ...
    NINN_DT %>% mutate(TOVT_SULYOZOTT = eval_tidy(parse_expr(expression2[num])) / 100 * TOVT * VGMA001_SULY) -> NINN_DT
    NINN_DT %>% group_by(M065_RETEG1, M0581_2J) %>% summarise(TOVT_SULYOZOTT = sum(TOVT_SULYOZOTT)) %>% arrange(M065_RETEG1, M0581_2J) -> NINN_Ordered
    CIS_INDICATOR <- expression2[num]
...
result_T10_DT <- rbind(Aggregate_INN, Aggregate_NINN)
result_T10_DT %>% group_by(DATAFLOW, FREQ, TIME_PERIOD, REF_AREA, TABLENAME, ACTIVITY, NUMBER_EMPL, INN_PF, INDICATOR, CIS_INDICATOR, UNIT_MEASURE, UNIT_MULT, DECIMALS) %>% summarise(OBS_VALUE = sum(OBS_VALUE)) -> result_T10_DT
# result_T10_DT <- result_T10_DT[, .(SUM_OBS_VALUE=sum(OBS_VALUE)), by = "DATAFLOW,FREQ,TIME_PERIOD,REF_AREA,TABLENAME,ACTIVITY,NUMBER_EMPL,INN_PF,INDICATOR,CIS_INDICATOR,UNIT_MEASURE,UNIT_MULT,DECIMALS"]
colnames(result_T10_DT)[14] <- "OBS_VALUE"

result_T10_DT_FINAL <- cbind(result_T10_DT[, 1:7], stringsAsFactors = FALSE)
result_T10_DT_FINAL$TYPE_ENT <- "_T"
result_T10_DT_FINAL <- cbind(result_T10_DT_FINAL, result_T10_DT[, c(8:10, 14, 11:13)], stringsAsFactors = FALSE)
result_T10_DT_FINAL$OBS_STATUS <- ""
result_T10_DT_FINAL$OBS_STATUS_1 <- ""
result_T10_DT_FINAL$CONF_STATUS <- ""
result_T10_DT_FINAL$COMMENT_OBS <- ""
result_T10_DT_FINAL <- as.data.frame(result_T10_DT_FINAL)
result_T10_DT_FINAL$OBS_VALUE <- as.character(result_T10_DT_FINAL$OBS_VALUE)
result_T10_DT_FINAL[, "OBS_VALUE"] <- gsub("\\.", ",", result_T10_DT_FINAL[, "OBS_VALUE"])

result_T10_DT_FINAL[result_T10_DT_FINAL$ACTIVITY == "A" | result_T10_DT_FINAL$ACTIVITY == "A01" | result_T10_DT_FINAL$ACTIVITY == "A02" | result_T10_DT_FINAL$ACTIVITY == "A03" | result_T10_DT_FINAL$ACTIVITY == "F" | result_T10_DT_FINAL$ACTIVITY == "G45" | result_T10_DT_FINAL$ACTIVITY == "G47" | result_T10_DT_FINAL$ACTIVITY == "I" | result_T10_DT_FINAL$ACTIVITY == "L" | result_T10_DT_FINAL$ACTIVITY == "M69" | result_T10_DT_FINAL$ACTIVITY == "M70" | result_T10_DT_FINAL$ACTIVITY == "M74" | result_T10_DT_FINAL$ACTIVITY == "M75" | result_T10_DT_FINAL$ACTIVITY == "N" | result_T10_DT_FINAL$ACTIVITY == "N77" | result_T10_DT_FINAL$ACTIVITY == "N78" | result_T10_DT_FINAL$ACTIVITY == "N79" | result_T10_DT_FINAL$ACTIVITY == "N80" | result_T10_DT_FINAL$ACTIVITY == "N81" | result_T10_DT_FINAL$ACTIVITY == "N82", "OBS_VALUE"] <- ""
result_T10_DT_FINAL <- subset(result_T10_DT_FINAL, result_T10_DT_FINAL$ACTIVITY != "GTN" | (result_T10_DT_FINAL$ACTIVITY == "GTN" & (result_T10_DT_FINAL$NUMBER_EMPL == "_T")))
# result_T10_DT_FINAL <- subset(result_T10_DT_FINAL, result_T10_DT_FINAL$ACTIVITY != "G" & result_T10_DT_FINAL$ACTIVITY != "M")

result_T10_DT_FINAL <- subset(result_T10_DT_FINAL, result_T10_DT_FINAL$TYPE_ENT == "INN")
...
set_TUR_Profile("TUR_PRD_NEW_ENT", "T10", T10_SDMX_FINAL_RESULT)
set_TUR_Profile("TUR_PRD_NEW_MKT", "T10", T10_SDMX_FINAL_RESULT)