...
for(num in 1:length(expression)){
  ...
  if(nrow(INN_DT) != 0){
    ...
    INN_DT %>% mutate(EXP24_SULYOZOTT = eval_tidy(parse_expr(expression3[num]))) -> INN_DT
    INN_DT %>% group_by(M065_RETEG1, M0581_2J) %>% summarise(EXP24_SULYOZOTT = sum(EXP24_SULYOZOTT)) %>% arrange(M065_RETEG1, M0581_2J) -> INN_Ordered
    CIS_INDICATOR <- expression2[num]
...
for(num in 1:length(expression)){
  ...
  if(nrow(NINN_DT) != 0){
    ...
    NINN_DT %>% mutate(EXP24_SULYOZOTT = eval_tidy(parse_expr(expression3[num]))) -> NINN_DT
    NINN_DT %>% group_by(M065_RETEG1, M0581_2J) %>% summarise(EXP24_SULYOZOTT = sum(EXP24_SULYOZOTT)) %>% arrange(M065_RETEG1, M0581_2J) -> NINN_Ordered
    CIS_INDICATOR <- expression2[num]
