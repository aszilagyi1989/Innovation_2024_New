...
expression <- c("is.na(INN$CLIM_GOV) == FALSE & INN$CLIM_GOV == 3", 
                "is.na(INN$CLIM_CUST) == FALSE & INN$CLIM_CUST == 3", 
                "is.na(INN$CLIM_PRIC) == FALSE & INN$CLIM_PRIC == 3", 
                "is.na(INN$CLIM_WEATH) == FALSE & INN$CLIM_WEATH == 3", 
                "is.na(INN$CLIM_GOV) == FALSE & INN$CLIM_GOV == 2", 
                "is.na(INN$CLIM_CUST) == FALSE & INN$CLIM_CUST == 2", 
                "is.na(INN$CLIM_PRIC) == FALSE & INN$CLIM_PRIC == 2", 
                "is.na(INN$CLIM_WEATH) == FALSE & INN$CLIM_WEATH == 2", 
                "is.na(INN$CLIM_GOV) == FALSE & INN$CLIM_GOV == 1", 
                "is.na(INN$CLIM_CUST) == FALSE & INN$CLIM_CUST == 1", 
                "is.na(INN$CLIM_PRIC) == FALSE & INN$CLIM_PRIC == 1", 
                "is.na(INN$CLIM_WEATH) == FALSE & INN$CLIM_WEATH == 1", 
                "is.na(INN$CLIM_GOV) == FALSE & INN$CLIM_GOV == 0", 
                "is.na(INN$CLIM_CUST) == FALSE & INN$CLIM_CUST == 0", 
                "is.na(INN$CLIM_PRIC) == FALSE & INN$CLIM_PRIC == 0", 
                "is.na(INN$CLIM_WEATH) == FALSE & INN$CLIM_WEATH == 0")

expression2 <- c("CLIM_GOV_3", 
                "CLIM_CUST_3", 
                "CLIM_PRIC_3", 
                "CLIM_WEATH_3", 
                "CLIM_GOV_2", 
...
expression <- c("is.na(NINN$CLIM_GOV) == FALSE & NINN$CLIM_GOV == 3", 
                "is.na(NINN$CLIM_CUST) == FALSE & NINN$CLIM_CUST == 3", 
                "is.na(NINN$CLIM_PRIC) == FALSE & NINN$CLIM_PRIC == 3", 
                "is.na(NINN$CLIM_WEATH) == FALSE & NINN$CLIM_WEATH == 3", 
                "is.na(NINN$CLIM_GOV) == FALSE & NINN$CLIM_GOV == 2", 
                "is.na(NINN$CLIM_CUST) == FALSE & NINN$CLIM_CUST == 2", 
                "is.na(NINN$CLIM_PRIC) == FALSE & NINN$CLIM_PRIC == 2", 
                "is.na(NINN$CLIM_WEATH) == FALSE & NINN$CLIM_WEATH == 2", 
                "is.na(NINN$CLIM_GOV) == FALSE & NINN$CLIM_GOV == 1", 
                "is.na(NINN$CLIM_CUST) == FALSE & NINN$CLIM_CUST == 1", 
                "is.na(NINN$CLIM_PRIC) == FALSE & NINN$CLIM_PRIC == 1", 
                "is.na(NINN$CLIM_WEATH) == FALSE & NINN$CLIM_WEATH == 1", 
                "is.na(NINN$CLIM_GOV) == FALSE & NINN$CLIM_GOV == 0", 
                "is.na(NINN$CLIM_CUST) == FALSE & NINN$CLIM_CUST == 0", 
                "is.na(NINN$CLIM_PRIC) == FALSE & NINN$CLIM_PRIC == 0", 
                "is.na(NINN$CLIM_WEATH) == FALSE & NINN$CLIM_WEATH == 0")
...
result_T29_DT <- rbind(Aggregate_INN, Aggregate_NINN)
result_T29_DT %>% group_by(DATAFLOW, FREQ, TIME_PERIOD, REF_AREA, TABLENAME, ACTIVITY, NUMBER_EMPL, INN_PF, INDICATOR, CIS_INDICATOR, UNIT_MEASURE, UNIT_MULT, DECIMALS) %>% summarise(OBS_VALUE = sum(OBS_VALUE)) -> result_T29_DT
# result_T29_DT <- result_T29_DT[, .(SUM_OBS_VALUE=sum(OBS_VALUE)), by = "DATAFLOW,FREQ,TIME_PERIOD,REF_AREA,TABLENAME,ACTIVITY,NUMBER_EMPL,INN_PF,INDICATOR,CIS_INDICATOR,UNIT_MEASURE,UNIT_MULT,DECIMALS"]
colnames(result_T29_DT)[14] <- "OBS_VALUE"

result_T29_DT_FINAL <- cbind(result_T29_DT[, 1:7], stringsAsFactors = FALSE)
result_T29_DT_FINAL$TYPE_ENT <- "_T"
result_T29_DT_FINAL <- cbind(result_T29_DT_FINAL, result_T29_DT[, c(8:10, 14, 11:13)], stringsAsFactors = FALSE)
result_T29_DT_FINAL$OBS_STATUS <- ""
result_T29_DT_FINAL$OBS_STATUS_1 <- ""
result_T29_DT_FINAL$CONF_STATUS <- ""
result_T29_DT_FINAL$COMMENT_OBS <- ""
result_T29_DT_FINAL <- as.data.frame(result_T29_DT_FINAL)
result_T29_DT_FINAL$OBS_VALUE <- as.character(result_T29_DT_FINAL$OBS_VALUE)
result_T29_DT_FINAL[, "OBS_VALUE"] <- gsub("\\.", ",", result_T29_DT_FINAL[, "OBS_VALUE"])

result_T29_DT_FINAL[result_T29_DT_FINAL$ACTIVITY == "A" | result_T29_DT_FINAL$ACTIVITY == "A01" | result_T29_DT_FINAL$ACTIVITY == "A02" | result_T29_DT_FINAL$ACTIVITY == "A03" | result_T29_DT_FINAL$ACTIVITY == "F" | result_T29_DT_FINAL$ACTIVITY == "G45" | result_T29_DT_FINAL$ACTIVITY == "G47" | result_T29_DT_FINAL$ACTIVITY == "I" | result_T29_DT_FINAL$ACTIVITY == "L" | result_T29_DT_FINAL$ACTIVITY == "M69" | result_T29_DT_FINAL$ACTIVITY == "M70" | result_T29_DT_FINAL$ACTIVITY == "M74" | result_T29_DT_FINAL$ACTIVITY == "M75" | result_T29_DT_FINAL$ACTIVITY == "N" | result_T29_DT_FINAL$ACTIVITY == "N77" | result_T29_DT_FINAL$ACTIVITY == "N78" | result_T29_DT_FINAL$ACTIVITY == "N79" | result_T29_DT_FINAL$ACTIVITY == "N80" | result_T29_DT_FINAL$ACTIVITY == "N81" | result_T29_DT_FINAL$ACTIVITY == "N82", "OBS_VALUE"] <- ""
result_T29_DT_FINAL <- subset(result_T29_DT_FINAL, ACTIVITY != "GTN" | (ACTIVITY == "GTN" & (NUMBER_EMPL == "_T")))
# result_T29_DT_FINAL <- subset(result_T29_DT_FINAL, ACTIVITY != "G" & ACTIVITY != "M")

result_T29_DT_FINAL <- subset(result_T29_DT_FINAL, TYPE_ENT == "_T")
...
result_T29_DT_FINAL[result_T29_DT_FINAL$ACTIVITY == "G" | result_T29_DT_FINAL$ACTIVITY == "M", "OBS_VALUE"] <- ""
...
write.table(rbind(result_T29_DT_FINAL, Aggregate_INN, Aggregate_NINN), T29_SDMX_FINAL_RESULT, sep = ";", quote = FALSE, row.names = FALSE, append = FALSE)

set_ENT_Profile("CLIM_CUST_0", "T29", T29_SDMX_FINAL_RESULT)
set_ENT_Profile("CLIM_CUST_1", "T29", T29_SDMX_FINAL_RESULT)
set_ENT_Profile("CLIM_CUST_2", "T29", T29_SDMX_FINAL_RESULT)
set_ENT_Profile("CLIM_CUST_3", "T29", T29_SDMX_FINAL_RESULT)
set_ENT_Profile("CLIM_GOV_0", "T29", T29_SDMX_FINAL_RESULT)
set_ENT_Profile("CLIM_GOV_1", "T29", T29_SDMX_FINAL_RESULT)
set_ENT_Profile("CLIM_GOV_2", "T29", T29_SDMX_FINAL_RESULT)
set_ENT_Profile("CLIM_GOV_3", "T29", T29_SDMX_FINAL_RESULT)
set_ENT_Profile("CLIM_PRIC_0", "T29", T29_SDMX_FINAL_RESULT)
set_ENT_Profile("CLIM_PRIC_1", "T29", T29_SDMX_FINAL_RESULT)
set_ENT_Profile("CLIM_PRIC_2", "T29", T29_SDMX_FINAL_RESULT)
set_ENT_Profile("CLIM_PRIC_3", "T29", T29_SDMX_FINAL_RESULT)
set_ENT_Profile("CLIM_WEATH_0", "T29", T29_SDMX_FINAL_RESULT)
set_ENT_Profile("CLIM_WEATH_1", "T29", T29_SDMX_FINAL_RESULT)
set_ENT_Profile("CLIM_WEATH_2", "T29", T29_SDMX_FINAL_RESULT)
set_ENT_Profile("CLIM_WEATH_3", "T29", T29_SDMX_FINAL_RESULT)