...
expression <- c("BPCS_ONL", "PRD_BPCS_ONL", "PRD_ONL", "INNO_PRD", "INN_X_RND", 
                ...
                "INNO_PCS_SLS_SERV_NOT_ZERO", "INNO_BPCS") # , "INN_ITR"
...
for(num in 1:length(expression)){
  ...
  T01_DT <- T01_DT[, .(ENT24_SULYOZOTT=sum(VGMA001_SULY), TOVT_SULYOZOTT=sum(TOVT * VGMA001_SULY), EMPL_SULYOZOTT=sum(EMPL * VGMA001_SULY)), by = "M065_RETEG1,M0581_2J"]
...
INN_DT <- INN_DT[, .(ENT24_SULYOZOTT=sum(VGMA001_SULY), TOVT_SULYOZOTT=sum(TOVT * VGMA001_SULY), EMPL_SULYOZOTT=sum(EMPL * VGMA001_SULY)), by = "M065_RETEG1,M0581_2J"]
...
NINN_DT <- NINN_DT[, .(ENT24_SULYOZOTT=sum(VGMA001_SULY), TOVT_SULYOZOTT=sum(TOVT * VGMA001_SULY), EMPL_SULYOZOTT=sum(EMPL * VGMA001_SULY)), by = "M065_RETEG1,M0581_2J"]
...
# INN_NINN_T01_DT %>% group_by(DATAFLOW, FREQ, TIME_PERIOD, REF_AREA, TABLENAME, ACTIVITY, NUMBER_EMPL, INN_PF, INDICATOR, CIS_INDICATOR, UNIT_MEASURE, UNIT_MULT, DECIMALS) %>% summarise(OBS_VALUE = sum(OBS_VALUE)) -> INN_NINN_T01_DT
...
colnames(INN_NINN_T01_DT)[14] <- "OBS_VALUE"
...
set_EMPL_Profile("EMPL", "T01", T1_SDMX_RESULT)
set_ENT_T1_Profile("", "T01", T1_SDMX_RESULT)
set_TOVT_Profile("TOVT", "T01", T1_SDMX_RESULT)